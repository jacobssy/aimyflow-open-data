...
[AimyFlow で完全版の役割ページを開く](https://www.aimyflow.com/ja/role/it-infrastructure-manager)
...
ITインフラマネージャー 向けのこのページでは、関連する AI ツール 175 件と主要スキル 5 件をまとめ、GitHub 上で素早く全体像を確認できるようにしています。特に インフラ設計, 運用信頼性管理, クラウド・DC管理 の観点が目立ち、最新の投票や詳細な workflow は AimyFlow 本体で確認できます。

## 主要スキル

- **インフラ設計**: 業務基盤となる拡張性あるインフラを設計する。
- **運用信頼性管理**: 監視、容量計画、パッチ、バックアップ、障害対応で安定稼働を守る。
- **クラウド・DC管理**: クラウドとオンプレミスをまたぐハイブリッド基盤を安定運用する。
- **セキュリティ統制**: アクセス制御、ハードニング、ログ、監査統制で要件に適合させる。
- **災害復旧計画**: 重要サービスを迅速に復旧するための戦略、切替手順、訓練計画を定める。
...
- [コミュニティ投票を見る](https://www.aimyflow.com/ja/role/it-infrastructure-manager)
