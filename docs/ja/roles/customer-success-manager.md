# カスタマーサクセスマネージャー

[AimyFlow で完全版の役割ページを開く](https://www.aimyflow.com/ja/role/customer-success-manager?utm_source=github&utm_medium=repo&utm_campaign=open_data&utm_content=role_doc_top_ja)

## 概要

- 一致ツール数: 14
- スキルカード数: 5

## 主要スキル

- **導入設計**: 導入計画と定着目標を設計し、立ち上がりを成功へ導く。
- **活用分析**: 利用状況や機能定着を分析し、活用課題と次の施策を見極める。
- **更新計画**: 価値実現や商談リスク、時期を踏まえて更新計画を組み立てる。
- **エスカレーション管理**: 製品・サービス・サポートの重大課題を調整し、顧客成果と信頼を守る。
- **サクセスプラン策定**: 測定可能な成果目標、定例レビュー、実行計画を事業目標に合わせて定める。

## 関連ツール

- [capalyze-ai](https://www.aimyflow.com/ja/ai/capalyze-ai?utm_source=github&utm_medium=repo&utm_campaign=open_data&utm_content=role_doc_tool_ja): Capalyzeは、複数ソースのデータとWebクローリング情報を統合して実用的なインサイトを導くデータ分析プラットフォームです。分析担当者や意思決定者の手作業を減らし、トレンド把握と意思決定を加速します。
- [creativefabrica-com](https://www.aimyflow.com/ja/ai/creativefabrica-com?utm_source=github&utm_medium=repo&utm_campaign=open_data&utm_content=role_doc_tool_ja): Creative Fabricaは、フォント・グラフィック・クラフト・デザイン素材を扱うデジタルマーケットプレイス兼サブスク基盤です。メーカーやデザイナー、POD販売者の商品制作を効率化し、AI時代の発想・カスタマイズ・制作スピードを高めます。
- [demodazzle-com](https://www.aimyflow.com/ja/ai/demodazzle-com?utm_source=github&utm_medium=repo&utm_campaign=open_data&utm_content=role_doc_tool_ja): DemoDazzleは、製品操作の記録、AIアバタープレゼン、インタラクティブなアプリデモ作成を支援するデモ自動化プラットフォームです。営業やプロダクトマーケティングの個別デモを、手動説明を繰り返さず拡張できます。
- [generate-folderly-com](https://www.aimyflow.com/ja/ai/generate-folderly-com?utm_source=github&utm_medium=repo&utm_campaign=open_data&utm_content=role_doc_tool_ja): Folderly EmailGenは、実績あるテンプレートからパーソナライズされたコールドメールを作成し、スパムチェックも行うAIメール生成ツールです。営業・マーケティングのアウトバウンド効率を高めます。
- [ifoto-ai](https://www.aimyflow.com/ja/ai/ifoto-ai?utm_source=github&utm_medium=repo&utm_campaign=open_data&utm_content=role_doc_tool_ja): iFotoは、画像補正、背景変更、高品質なビジュアル作成をオンラインで行えるAI写真編集スタジオです。EC事業者やマーケター、クリエイターの制作作業を効率化します。
- [instruct-ai](https://www.aimyflow.com/ja/ai/instruct-ai?utm_source=github&utm_medium=repo&utm_campaign=open_data&utm_content=role_doc_tool_ja): Instructは、企業がプレーンな英語でカスタムAIエージェントを作成し、複雑なワークフローを自動化できるノーコードプラットフォームです。運用やIT部門が開発負荷を抑えて自動化を導入できます。
- [intryc-com](https://www.aimyflow.com/ja/ai/intryc-com?utm_source=github&utm_medium=repo&utm_campaign=open_data&utm_content=role_doc_tool_ja): Intrycは、サポートチケットのレビュー、担当者品質の改善、実行可能なインサイト抽出を支援する顧客体験プラットフォームです。QAやCXチームが指導と品質向上を効率的に進められます。
- [marblism-com](https://www.aimyflow.com/ja/ai/marblism-com?utm_source=github&utm_medium=repo&utm_campaign=open_data&utm_content=role_doc_tool_ja): Marblismは、受信箱管理、SNS投稿、SEO記事作成、リード獲得を支援するAI社員サービスです。小規模チームや成長企業の定型業務を効率化します。

## AimyFlow で続きを見る

- [すべての AI ツールを見る](https://www.aimyflow.com/ja/explore?utm_source=github&utm_medium=repo&utm_campaign=open_data&utm_content=role_doc_explore_ja)
- [すべての役割を見る](https://www.aimyflow.com/ja/roles?utm_source=github&utm_medium=repo&utm_campaign=open_data&utm_content=role_doc_roles_ja)
- [コミュニティ投票を見る](https://www.aimyflow.com/ja/role/customer-success-manager?utm_source=github&utm_medium=repo&utm_campaign=open_data&utm_content=role_doc_votes_ja)

## 補足

- この GitHub ページは意図的に簡潔です。コミュニティ投票、多言語ページ、workflow コンテンツは AimyFlow 本体にあります。

