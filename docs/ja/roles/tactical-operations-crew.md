# 戦術作戦要員

[AimyFlow で完全版の役割ページを開く](https://www.aimyflow.com/ja/role/Tactical-Operations-Crew)

## 概要

- 一致ツール数: 2
- スキルカード数: 5

## 主要スキル

- **任務計画**: 目標や地形、脅威に応じて実行可能な任務計画を立てる。
- **脅威評価**: 敵対行動や脆弱性、事態拡大の危険を評価して戦術判断につなげる。
- **戦術機動**: 変化する作戦環境で安全に移動、配置、遮蔽を実施する。
- **交戦規定運用**: 法的要件と作戦上の制約の中で武力行使、拘束、捜索を行う。
- **装備即応管理**: 武器、防護装備、通信機器、任務装備を即応状態に維持する。

## 関連ツール

- [aifaceswap-io](https://www.aimyflow.com/ja/ai/aifaceswap-io): AI Face Swapは、登録不要で写真の顔入れ替えをすぐ試せるブラウザ型フェイススワップツールです。AI時代に、エンタメやSNS向けの軽量な画像実験をより手軽かつ高速にします。
- [replit-com](https://www.aimyflow.com/ja/ai/replit-com): Replitは、ブラウザ上でアプリやWebサイトを構築できるAI開発プラットフォームです。主に開発者、起業家、素早く開発したいチーム向けで、アイデアから試作品、デプロイまでの流れを短縮します。

## AimyFlow で続きを見る

- [すべての AI ツールを見る](https://www.aimyflow.com/ja/explore)
- [すべての役割を見る](https://www.aimyflow.com/ja/roles)
- [コミュニティ投票を見る](https://www.aimyflow.com/ja/role/Tactical-Operations-Crew)

## 補足

- この GitHub ページは意図的に簡潔です。コミュニティ投票、多言語ページ、workflow コンテンツは AimyFlow 本体にあります。

