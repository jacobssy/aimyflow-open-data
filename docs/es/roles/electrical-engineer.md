# Ingeniero eléctrico

[Abrir la página completa del rol en AimyFlow](https://www.aimyflow.com/es/role/electrical-engineer)

## Resumen

- Herramientas asociadas: 4
- Tarjetas de habilidades: 5

## Habilidades clave

- **Diseño de circuitos**: Diseña circuitos analógicos, digitales o de potencia que cumplen objetivos eléctricos, térmicos y de rendimiento.
- **Análisis de sistemas eléctricos**: Calcula cargas, fallas, ajustes de protección y comportamiento de distribución para una operación segura y confiable.
- **Pruebas eléctricas**: Valida equipos mediante mediciones, planes de prueba y diagnóstico para confirmar rendimiento y seguridad.
- **Cumplimiento normativo**: Aplica códigos eléctricos, normas de seguridad y prácticas documentales exigidas para aprobación e implementación.
- **Integración de sistemas de control**: Integra sensores, actuadores, variadores y controles basados en PLC en sistemas eléctricos funcionales.

## Herramientas relacionadas

- [bolt-new](https://www.aimyflow.com/es/ai/bolt-new): Bolt es un creador de apps con IA que ayuda a convertir prompts en aplicaciones web funcionales rápidamente, ideal para founders, equipos de producto y desarrolladores que prototipan ideas. En la era de la IA, acorta el camino de concepto a prototipo para validar productos más rápido.
- [clawxiv-org](https://www.aimyflow.com/es/ai/clawxiv-org): clawXiv es un archivo y servicio de distribución gratuito de acceso abierto para la investigación sobre agentes autónomos de IA, que ayuda a agentes e investigadores a buscar, explorar y contribuir con preprints en distintas áreas técnicas. Para investigadores de IA y desarrolladores de agentes, ofrece un espacio estructurado para dar visibilidad a hallazgos generados por agentes y seguir trabajos emergentes en sistemas de investigación autónoma.
- [createanything-com](https://www.aimyflow.com/es/ai/createanything-com): Anything es un creador de apps con IA que ayuda a convertir ideas en sitios web, apps, herramientas y productos digitales describiendo lo que se quiere, pensado para founders, makers y perfiles no técnicos. En la era de la IA, permite probar y lanzar conceptos más rápido con menos dependencia de ingeniería.
- [openrouter-ai](https://www.aimyflow.com/es/ai/openrouter-ai): OpenRouter es una API unificada y plataforma de enrutamiento para modelos de lenguaje que ayuda a desarrolladores a acceder, comparar y usar múltiples modelos desde una sola interfaz. Ofrece flexibilidad para optimizar costo, rendimiento y fiabilidad según las necesidades.

## Continuar en AimyFlow

- [Explorar todas las herramientas IA](https://www.aimyflow.com/es/explore)
- [Explorar todos los roles](https://www.aimyflow.com/es/roles)
- [Ver la votación de la comunidad](https://www.aimyflow.com/es/role/electrical-engineer)

## Notas

- Esta página de GitHub es intencionalmente breve. Las votaciones de la comunidad, las variantes multilingües y los workflows completos viven en el sitio principal de AimyFlow.

