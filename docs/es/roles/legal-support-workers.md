# Personal de apoyo legal

[Abrir la página completa del rol en AimyFlow](https://www.aimyflow.com/es/role/Legal-Support-Workers)

## Resumen

- Herramientas asociadas: 2
- Tarjetas de habilidades: 5

## Habilidades clave

- **Preparación de documentos legales**: Redacta, da formato y revisa formularios, escritos y documentos de apoyo con precisión y completitud.
- **Gestión de expedientes**: Mantiene expedientes ordenados, controla plazos y asegura que cada documento quede en el asunto correcto.
- **Tramitación ante tribunales**: Prepara y presenta escritos conforme a las reglas del tribunal, los plazos y los requisitos de notificación.
- **Apoyo a la investigación jurídica**: Localiza normas, reglamentos, referencias jurisprudenciales y antecedentes para apoyar el trabajo jurídico.
- **Gestión de registros confidenciales**: Gestiona información sensible de clientes y asuntos conforme a las normas de privacidad y control de acceso.

## Herramientas relacionadas

- [lyra-so](https://www.aimyflow.com/es/ai/lyra-so): Lyra es una plataforma de reuniones con IA que ayuda a los equipos a registrar llamadas, entender el contexto del negocio y convertir conversaciones en seguimientos accionables automáticamente.
- [podcast-adobe-com](https://www.aimyflow.com/es/ai/podcast-adobe-com): Adobe Podcast es una herramienta web de grabación y edición de audio con IA para grabar, transcribir, mejorar y compartir contenido hablado, ideal para podcasters, creadores y equipos de voz. Reduce el tiempo de limpieza y edición para publicar audio más claro más rápido.

## Continuar en AimyFlow

- [Explorar todas las herramientas IA](https://www.aimyflow.com/es/explore)
- [Explorar todos los roles](https://www.aimyflow.com/es/roles)
- [Ver la votación de la comunidad](https://www.aimyflow.com/es/role/Legal-Support-Workers)

## Notas

- Esta página de GitHub es intencionalmente breve. Las votaciones de la comunidad, las variantes multilingües y los workflows completos viven en el sitio principal de AimyFlow.

