# Anthropologe

[Vollständige Rollen-Seite auf AimyFlow öffnen](https://www.aimyflow.com/de/role/anthropologist?utm_source=github&utm_medium=repo&utm_campaign=open_data&utm_content=role_doc_top_de)

## Überblick

- Zugeordnete Tools: 2
- Skill-Karten: 5

## Kernkompetenzen

- **Ethnografische Feldforschung**: Führt teilnehmende Beobachtung und Interviews im Feld durch, um soziales Leben im Kontext zu dokumentieren.
- **Kulturanalyse**: Interpretiert Rituale, Normen, Verwandtschaft und Symbole, um Sinnordnungen innerhalb einer Gruppe zu erklären.
- **Kodierung qualitativer Daten**: Kodiert Feldnotizen, Transkripte und Artefakte zu Themen, die belastbare anthropologische Deutungen stützen.
- **Forschungsethik**: Gewährleistet informierte Einwilligung, Vertraulichkeit und Schutz der Gemeinschaft bei Forschung mit Menschen und Kulturgütern.
- **Vergleichende Interpretation**: Vergleicht Befunde zwischen Gesellschaften oder Epochen, um Muster, Unterschiede und historisch fundierte Erklärungen zu erkennen.

## Passende Tools

- [createanything-com](https://www.aimyflow.com/de/ai/createanything-com?utm_source=github&utm_medium=repo&utm_campaign=open_data&utm_content=role_doc_tool_de): Anything ist ein KI-App-Builder, der Ideen per Beschreibung in Websites, Apps, Tools und digitale Produkte verwandelt, ideal für Gründer, Maker und Non-Tech-Builder.
- [openrouter-ai](https://www.aimyflow.com/de/ai/openrouter-ai?utm_source=github&utm_medium=repo&utm_campaign=open_data&utm_content=role_doc_tool_de): OpenRouter ist eine einheitliche API- und Routing-Plattform für große Sprachmodelle, mit der Entwickler mehrere Modelle über eine einzige Schnittstelle nutzen, vergleichen und verwalten können.

## Auf AimyFlow fortfahren

- [Alle KI-Tools ansehen](https://www.aimyflow.com/de/explore?utm_source=github&utm_medium=repo&utm_campaign=open_data&utm_content=role_doc_explore_de)
- [Alle Rollen durchsuchen](https://www.aimyflow.com/de/roles?utm_source=github&utm_medium=repo&utm_campaign=open_data&utm_content=role_doc_roles_de)
- [Community-Votings ansehen](https://www.aimyflow.com/de/role/anthropologist?utm_source=github&utm_medium=repo&utm_campaign=open_data&utm_content=role_doc_votes_de)

## Hinweise

- Diese GitHub-Seite ist bewusst kurz gehalten. Community-Votings, mehrsprachige Varianten und Workflow-Inhalte liegen auf der Hauptseite von AimyFlow.

